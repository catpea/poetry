...
const last = fs.readdirSync(path.resolve(options.db.path), { withFileTypes: true }) .filter(o => o.isFile()) .map(o => o.name) .filter(s => s.endsWith('.html')) .sort() .pop()
